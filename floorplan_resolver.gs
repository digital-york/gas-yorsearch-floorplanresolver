...
function doGet(e) {
  ...
  (4) Possible improvement: Send email when 1 of the 3 possible error pages are landed upon
  ...
  Note: Javascript MUST be turned on in both NATIVE and SANDBOX mode for the script to work. This isn't a limitation of this script, more of GA scripts in general. 
        ...
        No point using the caching service to cache the response because 9 times out of 10 it will be different every time
  ...
  */
  ...
  if (skipHttp == true) { 
    ...
    imageLink = "//placeholdit.imgix.net/~text?txtsize=20&txt=No+valid+URL+params&w=300&h=300"
  } else {
    var heading,
    bodyText,
    imageLink,
  ...
  var response = UrlFetchApp.fetch(url);
    if (response.getResponseCode() === 200) {
      ...
      if (objectContent.table.rows[0] && !objectContent.table.rows[1]) { 
        ...
        return [false, "Floorplan not found", "We are very sorry but we were unable to locate a floorplan at this time (multiple rows returned).", "//placeholdit.imgix.net/~text?txtsize=20&txt=No+floorplan+found&w=300&h=300" ]
      }
      else { return [false, "Floorplan not found", "We are very sorry but we were unable to locate a floorplan at this time.", "//placeholdit.imgix.net/~text?txtsize=20&txt=No+floorplan+found&w=300&h=300" ] }
